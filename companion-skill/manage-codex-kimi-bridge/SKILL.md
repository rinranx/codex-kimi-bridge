...
Codex Desktop's provider-private `encrypted_content` remains opaque and cannot be decrypted by the bridge. Version `0.4.1` uses two user-trusted Codex Hooks to capture a visible task before private wrapping and carry it through a locally signed `CKB1` envelope. It also records signed Kimi targets locally so an unsafe later cross-provider follow-up can be denied before it reaches the child:

1. Run `codex-kimi-bridge hooks status --json`. This is read-only. Require both managed hooks before using the signed path. The `PreToolUse` matcher must cover the known spawn names `Agent`, `spawn_agent`, `collaborationspawn_agent`, and separator-based `collaboration.spawn_agent` compatibility forms, plus the equivalent `send_message` and `followup_task` names used by the follow-up guard. Multi-Agent V2 may flatten the namespace by removing the separator.
...
5. After the child starts, use only `wait_agent` to wait for progress and automatic final-result delivery. Never call `send_message` or `followup_task` on the running Kimi child. Version `0.4.1` denies those calls for a registered Kimi target before delivery because Codex otherwise wraps the follow-up body in opaque provider state and can terminate the child sampling turn.
6. If the task genuinely needs new instructions, ask for a new visible user request containing a complete `[KIMI_TASK]`, then create a new uniquely named `kimi_frontend` child. Do not replay the earlier prompt cache into an existing child.
7. Let Kimi report progress as ordinary assistant commentary and finish with an ordinary final answer. Do not ask it to call `send_message` or `followup_task` back to an OpenAI parent; prefer automatic final-result delivery.
8. For a Kimi-created `kimi_frontend` descendant, put the exact descendant task between `[KIMI_TASK]` markers in the `spawn_agent.message`. This explicit marked tool input can be signed without a new `UserPromptSubmit` event.
...
The envelope authenticates source and integrity; it is not encryption. Visible prompts and hashed Kimi-target records are cached with mode `0600` under `~/Library/Caches/codex-kimi-bridge/handoff-v1`; the directory uses mode `0700`, stale files are cleaned after 24 hours, and envelopes/target registrations expire after six hours. Never describe this as decryption or confidential storage.
...
- `unsupported_content_part` for `encrypted_content`: version 0.3.1 accepts `agent_message` but rejects this part. Version 0.3.2 and later safely omit opaque values. Version `0.4.0` and later recognize only their own `CKB1` envelope after HMAC, recipient, agent-type, and expiry verification; they still never decrypt or forward OpenAI provider state.
- `unsupported_cross_provider_followup`: the parent tried `send_message` or `followup_task` on an existing Kimi child. In `0.4.1`, a trusted updated `PreToolUse` hook should deny this before delivery for registered Kimi targets. Stop retrying, wait for automatic completion, or submit a new visible `[KIMI_TASK]` and create a new child. If the error came from the bridge instead of the hook, reinstall the `0.4.1` hooks, restart Desktop, and review/trust the changed command.
- `missing_handoff_envelope` or Kimi returns `Payload 为空` during the initial `NEW_TASK`: check the running version, then run `hooks status --json`. Confirm the current user request triggered `UserPromptSubmit`, Desktop trusts both hooks, and `task_name` matches the child recipient. Do not work around a failed signature by forwarding the raw encrypted value. A later `MESSAGE` with an empty payload is a follow-up problem, not an initial handoff problem.
...
- Recursive subagents: namespace translation remains available, but cross-provider `send_message` and `followup_task` are not a safe result channel because their content may be provider-private. Version `0.4.1` blocks those calls when their target matches a Kimi child registered by the same parent session. Prefer `wait_agent` and automatic final-result delivery. A Kimi-created `kimi_frontend` descendant must put its exact task in `[KIMI_TASK]` markers inside the spawn message so the hook can sign it without a user-prompt cache. The bridge does not impose its own depth or concurrency limit.
...
For protocol changes, also compare both implementations against `compat/responses-request.json`. For `agent_message`, require unit coverage for text, multimodal content, route-metadata injection rejection, omission of internal passthrough metadata, omission of every unknown string and non-string `encrypted_content` value, unchanged translation when only opaque provider state changes, valid signed handoff delivery, tampered/expired/wrong-recipient rejection, initial empty-payload fail-closed behavior, dedicated later-follow-up classification, marked recursive handoff without a prompt cache, and targeted `send_message`/`followup_task` denial without affecting non-Kimi targets. Test hook configuration merge/update/uninstall without touching the user's real hooks file. Require terminal assistant text to carry `phase = "final_answer"` and tool-progress text to carry `phase = "commentary"`. Report all checks and explicitly state whether a real Codex `spawn_agent`/live Kimi request was skipped.
