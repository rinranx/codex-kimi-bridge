...
async function serveCommand(flags, io) {
  ...
  if (!quiet) {
    io.stderr.write(
      `codex-kimi-bridge ${VERSION} listening on http://${host}:${address.port}\n`,
    );
...
async function doctorCommand(flags, io, json) {
  ...
  const checks = {
    ...
    bind: {
      ok: portAvailable || localService?.service === "codex-kimi-bridge",
      host,
      port,
  ...
  if (json) {
    ...
    io.stdout.write(`codex-kimi-bridge ${VERSION}\n`);
    io.stdout.write(`Node.js: ${checks.node.ok ? "OK" : "FAIL"} (${checks.node.version})\n`);
    io.stdout.write(
      `Port ${host}:${port}: ${portAvailable ? "available" : localService?.service === "codex-kimi-bridge" ? "bridge already running" : "occupied"}\n`,
    );
...
function helpText() {
  return `codex-kimi-bridge ${VERSION}
