...
### `kimi_frontend` 角色指令示例

模型参数决定子代理调用哪个 Kimi 模型，`developer_instructions` 则定义它要扮演的角色、检查重点和输出格式。随附的 `install/templates/kimi_frontend.toml` 已包含下面这套可以直接使用的前端审查示例：
...
这段内容只是默认角色示例，可以按你的任务改成代码审查、产品体验分析或其他专长。若只希望 Kimi 提供建议，请同时保留 `sandbox_mode = "read-only"` 和“默认只读”的指令。
