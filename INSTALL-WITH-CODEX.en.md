...
> The current stable release is `v0.4.0`. Use only that version's GitHub Release and checksum files; never substitute `main/downloads`, a similarly named package, or an older build without signed handoff.
...
1. Read README.en.md, INSTALL-WITH-CODEX.en.md, install/INSTALL-GUIDE.en.md, SECURITY.md, and the SHA-256 files attached to GitHub Release v0.4.0 before installing.
2. Install the default Rust single-binary codex-kimi-bridge 0.4.0. Do not install similarly named npm packages or treat the node/ fallback as the default. A new installation requires no Rust, Node.js, or npm.
3. Download the macOS kit only from this repository's versioned GitHub Release v0.4.0, verify SHA-256, and install it at ~/.local/bin/codex-kimi-bridge without sudo. Do not obtain build artifacts from main/downloads.
...
11. Run codex-kimi-bridge hooks install to merge rather than replace ~/.codex/hooks.json and confirm a timestamped backup. Confirm that the PreToolUse matcher covers Agent, spawn_agent, Multi-Agent V2's normalized collaborationspawn_agent name, and collaboration.spawn_agent compatibility forms. Ask me to restart Desktop, open `/hooks`, review, and trust UserPromptSubmit plus PreToolUse; never bypass hook trust. Explain that CKB1 uses HMAC signing, not encryption, and tasks must not contain secrets.
...
15. doctor --live, request, and a real spawn_agent acceptance test make Kimi calls and consume quota. Ask for my separate explicit approval immediately before any of them. If I do not approve, skip them and state that no live call was made.
...
- `~/.local/bin/codex-kimi-bridge --version` prints `0.4.0`
