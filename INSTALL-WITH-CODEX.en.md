...
> The current stable release is `v0.4.1`. Use only that version's GitHub Release and checksum files; never substitute `main/downloads`, a similarly named package, or an older build without signed handoff.
...
1. Read README.en.md, INSTALL-WITH-CODEX.en.md, install/INSTALL-GUIDE.en.md, SECURITY.md, and the SHA-256 files attached to GitHub Release v0.4.1 before installing.
2. Install the default Rust single-binary codex-kimi-bridge 0.4.1. Do not install similarly named npm packages or treat the node/ fallback as the default. A new installation requires no Rust, Node.js, or npm.
3. Download the macOS kit only from this repository's versioned GitHub Release v0.4.1, verify SHA-256, and install it at ~/.local/bin/codex-kimi-bridge without sudo. Do not obtain build artifacts from main/downloads.
...
11. Run codex-kimi-bridge hooks install to merge rather than replace ~/.codex/hooks.json and confirm a timestamped backup. Confirm that the PreToolUse matcher covers initial spawn plus ordinary, Multi-Agent V2 flattened, and separator-based `send_message` / `followup_task` names. Ask me to restart Desktop, open `/hooks`, review, and trust UserPromptSubmit plus PreToolUse; never bypass hook trust. Explain that CKB1 uses HMAC signing, not encryption, and tasks must not contain secrets.
...
13. Install the bundled install-launchagent.command only if I choose C. Confirm that its plist uses the absolute ~/.local/bin/codex-kimi-bridge path, remains loopback-only, and explain uninstall-launchagent.command. A Kimi API Open Platform setup must not reuse the default Kimi Code LaunchAgent arguments unchanged.
14. On startup, require output containing implementation: rust. First run version, checksum, health, doctor --json, hooks status --json, and offline translate-request checks that do not contact Kimi.
15. doctor --live, request, and a real spawn_agent acceptance test make Kimi calls and consume quota. Ask for my separate explicit approval immediately before any of them. After a real Kimi child starts, use only `wait_agent`; never call `send_message` or `followup_task` on it. If new instructions are needed, ask me for a new complete visible `[KIMI_TASK]` and create a newly named child. If I do not approve, skip live requests and state that none were made.
16. Suggest node/ only after confirming a Rust compatibility problem. Its command must be codex-kimi-bridge-node, and it cannot listen on 8787 at the same time as Rust.
17. At completion report: downloaded file and SHA-256, installed path and version, key type, model, upstream, configuration files changed and backed up, Multi-agent v2 status, both hooks' installation and trust state, my selected startup method, whether the bridge is running, how to start it next time, and whether any real Kimi request was sent.
```

## Expected result

- `~/.local/bin/codex-kimi-bridge --version` prints `0.4.1`
...
- Both handoff hooks are installed and reviewed/trusted by the user in `/hooks`
