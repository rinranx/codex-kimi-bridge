...
1. 先完整阅读 README.md、INSTALL-WITH-CODEX.md、install/INSTALL-GUIDE.zh-CN.md、SECURITY.md，以及 GitHub Release v0.3.0 随附的 SHA-256 校验文件。
2. 默认安装 Rust 单文件版 codex-kimi-bridge 0.3.0。不要安装名字相似的 npm 包，也不要把 node/ 回退版当成默认版。新安装不要求 Rust、Node.js 或 npm。
3. 只从本仓库固定版本的 GitHub Release v0.3.0 下载 macOS 安装包，先核对 SHA-256，再安装到 ~/.local/bin/codex-kimi-bridge。不要使用 sudo，也不要从 main/downloads 获取构建产物。
...
- `~/.local/bin/codex-kimi-bridge --version` 输出 `0.3.0`
...
若 Codex 无法读取 GitHub，可先手动下载[完整 macOS 安装包](https://github.com/rinranx/codex-kimi-bridge/releases/download/v0.3.0/codex-kimi-bridge-macos-install-kit-0.3.0.zip)，把解压后的文件夹交给它，并把提示词第一句改为“请从我提供的本地安装包安装”。
