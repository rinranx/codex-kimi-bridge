...
Release downloads include SHA-256 checksums. The `0.2.0-alpha.1` macOS binaries are not Apple-notarized; verify the checksum before bypassing a Gatekeeper warning. The universal installation kit contains the same Rust binary for Apple Silicon and Intel Macs.
